...
11. Session view loads `report.md` data / `tasks.json` and the video file via the directory handle, renders interactive player + task list.

## Editing the analysis (v1.1)

The session view is no longer read-only. A mode switcher (icon-only, right of the Tasks/Markdown tabs; the mode picker is hidden on the Markdown tab and on archived runs) offers three modes on the **live** run:

- **View** — the original read-only experience.
- **Edit** — inline editing of every task field (title / description / screen_context text; category / priority enum dropdowns with the chevron inside the pill) plus the session **overview** (relocated to the top of the Tasks pane). Edits autosave on blur via a **non-archiving** save (`saveSessionEdits`): it writes `tasks.json` and re-renders `report.md` so the two never drift, and does **not** create a run. Add / delete / reorder tasks live here too. A field that diverges from the run's immutable AI baseline (`tasks.ai.json`) shows a quiet edited-dot; one **task-level** "revert to AI" (left of delete) restores a task. Manual edits are in-place — they never version.
- **Comment** — plannotator annotation: select text in a task field / overview to attach an anchored comment (`taskId` + field + quote), or add a **global** comment; stored in `comments.json`. Commented text and the pending selection are highlighted **yellow** (ADR-026). Anchors degrade gracefully (kept, tagged unanchored) when the quoted text is later edited.
